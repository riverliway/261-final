...
def preprocess_and_engineer_features_stateless(df):
    ...
    # df_clean = add_pagerank_features(df_clean)
...
def preprocess_and_engineer_features_stateful(df):
    ...
    """
    df_clean = generate_previous_flight_delay_minutes(df)
    df_clean = add_pagerank_features(df_clean)
...
# # small test (tmp)
# df_flights_weather_3m = get_data_from_cloud('OTPW_3M_2015.csv').limit(1000).cache()
# display(preprocess_and_engineer_features(df_flights_weather_3m))
