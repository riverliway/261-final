...
parquet_file_path = f"{team_blob_url}/Phase_2/Cleaned_Data_3m"
...
threshold = 1 #minutes
overrepresented_data = clean_data.filter(clean_data.dep_capped_delay < threshold)
underrepresented_data = clean_data.filter(clean_data.dep_capped_delay >= threshold)

# Downsample the overrepresented data to 20%
downsampled_overrepresented_data = overrepresented_data.sample(withReplacement=False, fraction=0.1, seed=123)

# Combine the downsampled overrepresented data with the underrepresented data
balanced_data = underrepresented_data.union(downsampled_overrepresented_data)

# Verify the new distribution
balanced_data.groupBy("dep_capped_delay").count().show()


# COMMAND ----------

departure_capped_delay_pd = balanced_data.select('dep_capped_delay').toPandas()
...
va_df = va_df.select(['features', 'dep_capped_delay', 'FL_DATE'])

# Choose a cutoff date for splitting the data
cutoff_date = "2015-03-15"
...
# MAGIC %md
# MAGIC ### Set up Evaluators in PySpark
# MAGIC
# MAGIC Evaluation metrics we'd like to include: 
# MAGIC
# MAGIC - RMSLE
# MAGIC - MAPE
# MAGIC - F3 ? 
...
def time_series_cv(data, date_col, model, evaluator, metric, folds=5, basis = 'rolling'):
    ...
    for fold in range(1,folds+1):
        ...
        #blocked
            ...
            train_data = data_with_row_number.filter((col("row_num") >= train_beg) & (col("row_num") <= train_end))
            validation_data = data_with_row_number.filter((col("row_num") > train_end) & (col("row_num") <= validation_end))
            #Set up for next iteration
            train_beg = train_end + 2

        #error out
        else:
            print('Please enter rolling or blocked as the basis setting.')
            break
    ...
    # Abdul's suggestion: 
    # use RMSE from the last fold (instead of avverging), which has the most training data, provides a more representative view of how the model will perform in a real-world data
...
cv_metrics, cv_coefficients, best_cvModel = time_series_cv(train, 'FL_DATE', cv_lr_model, cv_evaluator, 'rmse', basis = 'rolling')
...
#### WORKING ON FUNCTION!
# 
#  Define a function for time-series cross-validation on a BLOCKED BASIS
